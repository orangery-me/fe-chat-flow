...
function JoinedRooms ({ userId }) {
    ...
    const markAllNotiInARoomAsRead = async (roomId) => {
        const updatedNoti = noti.filter(notification => notification.chatRoomId != roomId);  // Loại bỏ thông báo trong room đã được đánh dấu là đã đọc
                ...
                return (
                    <div className="group" key={room.id}>
                        ...
                        <div className="group-item">
                            <div className="info">
                                <Link to={`/chat/${room.id}`} className="link" onClick={() => markAllNotiInARoomAsRead(room.id)}>
                                    {displayNames[room.id] || "Loading..."}
                                </Link>
                            </div>
                            <div className="last-message">
                                {Object.entries(lastMessages).map(([roomId, { sender, timestamp }]) => {
                                        ...
                                        return (
                                            <p key={roomId}>
                                                ...
                                                {(() => {
                                                    ...
                                                    const hoursDiff = Math.floor((currentDate - messageDate) / (1000 * 60 * 60));

                                                    // Trả về chuỗi thích hợp
                                                    if (hoursDiff == 0) {
                                                        const minutesDiff = Math.floor((currentDate - messageDate) / (1000 * 60));
                                                        if (minutesDiff == 0) {
                                                            return `vừa xong`;
                                                        } else {
                                                        ...
                                                        if (hoursDiff > 24) {
                                                            return `${Math.floor(hoursDiff / 24)} ngày trước`;
                                                        }
